...
class purchaseComparisonCreation(models.TransientModel):
    _name = 'purchase.comparison.creation.wizard'

    @api.model
    def default_get(self,fields_list):
        res = super(purchaseComparisonCreation, self).default_get(fields_list)
        active_ids = self.env.context.get('active_ids', [])

        vals = []
        for line in self.env['purchase.requisition.line'].browse(active_ids):
            if line.state != 'compare':
                raise UserError(u'Зөвхөн харьцуулалт хийх төлөвтэй шаардахын мөр дээр харьцуулалт үүсгэнэ.')
            if not line.product_id:
                raise UserError(u'Бараа сонгоогүй шаардахын мөр дээр харьцуулалт үүсгэх боломжгүй.')
            vals.append((0,0, {
                'product_id': line.product_id.id,
                'product_description': line.product_desc,
                'product_qty': line.product_qty,
                'market_value': line.product_price,
                'market_price_total': line.amount,
            }))
        res.update({
            'product_ids': vals
        })
    ...
    def create_comparison(self):
        ...
        # Шаардахын мөрүүдийг шинэчлэх
        active_ids = self.env.context.get('active_ids', [])
        for line in self.env['purchase.requisition.line'].browse(active_ids):
            line.write({
                'comparison_state': comparison_id.state,
                'comparison_id': comparison_id.id,
                'state': 'compared',
                # 'comparison_date_end': , #Харьцуулалт дууссан огноо
            })

        #Үнийн саналын хүсэлт үүсгэх
        mail_user_ids = []
        for line in self.partner_ids:
            existing_partners = [order_id.partner_id for order_id in comparison_id.order_ids]
...
class PurchaseComparisonCreationPartner(models.TransientModel):
    _name = 'purchase.comparison.creation.partner'
    ...
    comparison_id = fields.Many2one('purchase.comparison.creation.wizard',string = 'Comparison')
...
class PurchaseComparisonCreationProduct(models.TransientModel):
    _name = 'purchase.comparison.creation.product'
    ...
    def _onchange_product_id(self):
        if self.product_id:
            ...
            self.market_value = self.product_id.cost_price
